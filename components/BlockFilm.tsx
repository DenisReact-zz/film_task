...
const BlockFilm = (props) => {
    ...
    const changeActeveStar = () => {
        if(activeStar){
            setActiveStar(false);
            if(localStorage.getItem("markedFilms")){
                const arrDats = JSON.parse(localStorage.getItem("markedFilms"));
                localStorage.setItem("markedFilms", 
                `[${arrDats.filter(element => element !== dataFilm.id) }]`)
            }
        }else{
            setActiveStar(true);
            if(!localStorage.getItem("markedFilms")){
                localStorage.setItem("markedFilms", `[${dataFilm.id}]`)
            }else{
                const arrDats = JSON.parse(localStorage.getItem("markedFilms"));
                arrDats.push(dataFilm.id);
                localStorage.setItem("markedFilms", `[${arrDats}]`)
            }
        }
    } 

    const checkElementInLocalStorage = () => {
        const arrDats = JSON.parse(localStorage.getItem("markedFilms"));
        if(arrDats.includes(dataFilm.id)){
          setActiveStar(true)
          return true
        }
        return false
        ...
        <div className={s.blockFilm}>
            <div className={s.img} 
                ...
                <div className={s.starLink}>
                    ...
                    <div className={activeStar || checkElementInLocalStorage() ? `${s.star} ${s.active}` : s.star} onClick={changeActeveStar}>
